(function () {
/*//渲染新品首发，调用后端接口代码在516行，点击图片跳转在586行代码*/
  //  获取所有图片
  ...
  window.addEventListener('load', () => {
    ...
    class Floor {
      constructor(settings = {}) {
        ...
        // 1,2,3,4,5楼
        this.one = document.querySelector(settings.one);//内容区域
      ...
      render() {
        ...
        $.ajax({
          ...
          for (let i=0;i<data.length;i++){
            console.log("data.pname"+data[i].pname);
            strHTML += `
                <li>
                <div class="hd">
                  <a href="javascript:">
                    <img src="/images/product/${data[i].pfileName}" alt="">
                    <img src="/images/product/${data[i].pfileName}" alt="" data-id=${data[i].pid}>
                  </a>
                </div>
                <div class="bd">
                  <div class="prdtTags">
                  ...
                  <h4 class="name"><a href="javascript:" class="fs-14">${data[i].pname}</a></h4>
                  <div class="price">
                    <span class="price-new">${data[i].pprice}</span>
                    <span class="price-old">${data[i].pprice}</span>
                  </div>
                </div>
              </li>
            `
